<template>
	<div class="tab_home">
		<van-popup v-model="show" overlay-class="loginPage">
			<login-page/>
		</van-popup>
		<sign-board v-once v-if="shopInfo" :boardUrl="shopInfo.avatar" :storeName="shopInfo.shop_name" />
		<shop-info-group v-once v-if="shopInfo" class="interval_bot" :location="location" :address="shopInfo.address" :notice="shopInfo.notice" :mobile="shopInfo.contact" />
		<van-list v-model="loading" class="scroll-load" :finished="finished" :immediate-check="false" :offset="100" @load="loadMore">
			<item-group v-for="( group, key ) in itemGroup" v-if="group" :key="key" class="interval_bot" :setting="group.setting">
				<component v-for="(item, i) in group.items" :goods="item" :key="item.id" :is="getStyle(group.setting.style)" @click="toGoods(item)">
